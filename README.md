...
- **model ของหุ่นยนต์** ปัญหคาความไม่เข้าใจการตั้งค่า URDF เนื่องจาก solidwork set defualt value ไว้แล้วแต่โปรแกรมเข้าใจว่าเป็นทรงตันทำให้ต้องตั้งค่าใหม่ให้ inetia น้อยลงพร้อมกับค่าอื่นๆให้สอดคล้องกัน สุดท้ายแก้ไขโดยการเขียนโมเดล URDF ขึ้นใหม่เพื่อใช้งานชั่วคราว
- **การเรียนรู้การใช้งาน ros/gazebo/rviz และ ubuntu** เนื่องจากทั้งหมดเป็นเรื่องใหม่ที่ต้องเรียนรู้จึงใช้เวลาในระดับหนึ่ง
- **Gazebo** ไม่สามารถสร้างประตูได้ และเกิดปัญหา save world ไม่ได้ แก้ไขโดยการเปิด gazebo ด้วย sudo
### Phrase 3 (dual boost)
- **virtual machine** ไม่สามารถใช้งานได้ เมื่อทดลอง version ที่เก่ากว่าก็ไม่สามารถรันได้เช่นกัน แก้ไขโดยใช้การ dual boost ควบคู่ไปกับ window และเริ่มต้นใหม่
- **lidar** สามารถรับค่าได้แค่ด้านหน้าเพราะการออกแบบ แก้ไขโดยลดขนาดของหุ่นยนต์เพื่อวาง Lidar ไว้ด้านบนหัว
- **teleop** ทำงานอย่างที่ไม่เข้าใจ (phrase2 สร้างหุ่นยนต์เคลื่อนที่โดยใช้ bar เลื่อน)
  - หุ่นยนต์ไม่เดิน ในช่วงแรกที่ทดลองนำมาใช้กับหุ่นยนต์แต่ล้อของหุ่นยนต์กลับไม่หมุน เมื่อพบว่า ros รับคำสั่งจริงและ rostopic ตรงกัน ทำให้ต้องตรวจสอบการวางแกนต่างๆ  สุดท้ายพบว่ามีความผิดพลาดในการตั้งแกนจาก solidwork จึงกลับไปจัดการใหม่
  - หุ่นเดินไม่ตรงตามความต้องการ จึงเลือกใช้ turtlebot ในการเก็บค่าจากแมพเพื่อข้ามไปทำส่วนอื่นก่อน
- **Map** ที่ได้จากการ mapping ไม่ตรงกับ laser scan ของหุ่นที่ฉายใน gazebo พบว่าเกิดจากค่าในไฟล์ amcl.launch ไม่เป็นค่าเริ่มต้น
- เกิดปัญหาในการทำ **navigation**
  - หุ่นยนต์เดินชนผนังไม่ไปตามทางที่วาดเอาไว้ ในช่วงแรกเข้าใจว่าเป็นการออกแบบหุ่นยนต์ที่ผิดพลาดหรือล้อเล็กเกินไป จึงมีการทดลองออกแบบในหลายส่วน รวมไปถึงการทดลองการใช้จากไฟล์แบบอื่นๆ สุดท้ายแล้วเมื่อทดลองใช้ navigation จากturtlebot ก็เกิดผลลัพท์เช่นเดียวกัน นั่นคือหุ่นยนต์เดินติดกับผนังก่อนที่จะถึงจุดเลี้ยวเนื่องจาก path พยายามทำให้สั้นที่สุดแต่ configuration ของหุ่นไม่ดีพอ จึงเลือกที่จะเขียนกันจากไฟล์ python ให้หุ่นยนต์เดินเป็นเส้นตรงในจุดที่มีการเลี้ยวมากเกินไป
  - เมื่อทดลองใช้ model ที่ตัวเองสร้างกับพบว่าหุ่นหมุนมากเกินไปและออกนอกเส้นทางในการเดิน จึงต้องทดลองปรับค่าต่างๆเพื่อให์หุ่นยนต์เดินในทางที่เหมาะสมที่สุด แต่เมื่อทดลองพบว่าปัญหาต่างๆที่เจอลดลงบ้างแต่ไม่หมดไป จึงต้องกลับไปใช้ turtlebot อย่างที่ได้กล่าวไว้ตั้งแต่หัวข้อ getting ready (ตัวอย่างปัญหาที่พบหลังจากแก้ไขครั้งสุดท้าย: https://youtu.be/FOV9eAkC9Kk)
